...
public class DrugsUpdateServiceImpl implements DrugsUpdateService {
    ...
    public void initialUpdate() {
        ...
        // run database back-up following initial update
        databaseBackup();
    }
    ...
    public void scheduledUpdate() {
        // first run database back-up, before any changes made
        databaseBackup();
        // download Authorised Medicines list
        downloadAuthorisedList();
        // download Interchangeables list
    ...
    public void databaseBackup() {
        try {
            // specify the path to pg_dump and required database credentials
            String command = 
            "pg_dump -U postgres -h localhost -p 5432 -F c -b -v -f " + System.getProperty("user.dir") + "/database-backups/postgres.backup" + " postgres";

            // create process builder to execute the backup command
            ...
            // print result message to the console
            if (process.exitValue() == 0) {
                System.out.println("Backup completed successfully!");
            } else {
                System.err.println("Backup failed with exit code: " + process.exitValue());
            }
        } 
        catch (Exception e) {
            e.printStackTrace();
