document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("product-name-search").addEventListener("submit", async function(event) {
        ...
        // get the value submitted (the product name)
        const productName = document.getElementById("product-name").value;
        // start from page 0
        let currentPage = 0;

        async function getDrugsByProductName(page) {
        
            // add necessary query parameters
            const url = new URL("http://localhost:8080/drugs/search-by-product-name");
            ...
            try {
                ...
                // call backend search for drugs search by product name API
                ...
                if (response.ok) {
                    ...
                    if(response.status == 404){
                       messageContainer.innerHTML = "Product not found. Please check spelling and try again.";
                    }
                    else if(response.status == 204){
                        messageContainer.innerHTML = "No alternative medications found for this product.";
                    }
                    else{
        ...
        function displayDrugs(results) {
            ...
            if (!results.first) {
                ...
                prevButton.onclick = () => {
                    currentPage -= 1;
                    getDrugsByProductName(currentPage);
                };
