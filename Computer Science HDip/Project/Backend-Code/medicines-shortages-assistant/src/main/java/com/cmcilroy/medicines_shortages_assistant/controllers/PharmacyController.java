...
public class PharmacyController {
    ...
    public ResponseEntity<PharmacyDto> registerNewPharmacy(@RequestBody PharmacyDto pharmacyDto) {
        // map Dto to Entity
        PharmacyEntity pharmacyEntity = pharmacyMapper.mapFrom(pharmacyDto);
        // check if pharmacy account already exists in the database
        if(pharmacyService.isPresent(pharmacyEntity.getPsiRegNo())){
            // if record already exists, return HTTP status Conflict to indicate there’s a conflict with the resource
