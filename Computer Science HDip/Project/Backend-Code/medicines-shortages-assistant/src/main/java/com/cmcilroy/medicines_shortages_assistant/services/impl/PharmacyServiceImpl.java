...
public class PharmacyServiceImpl implements PharmacyService{
    ...
    public Optional<PharmacyEntity> signIn(AccountCredentialsDto credentials) {
        ...
        if(pharmacy.isLocked()){
            // check if the 5 minute lock time has expired
            if (pharmacy.getLockTimer().isBefore(LocalDateTime.now())) {
                // unlock the account
                pharmacy.unlockAccount();
            } else {
                // otherwise the account is still locked, throw an exception telling the user how many minutes lock remaining
                throw new LockedException(
                "Account locked, please try again in " + Duration.between(LocalDateTime.now(), pharmacy.getLockTimer()) + " minutes.");
            }
        }
        ...
        if(!passwordEncoder.matches(credentials.getPassword(), pharmacy.getPassword())) {
            // incorrect password, increment attempt counter
            pharmacy.incrementCounter();
            if(pharmacy.getAttemptCounter() < 3){
                throw new BadCredentialsException("Incorrect password. " + (3 - pharmacy.getAttemptCounter()) + " attempts remaining.");
            }
            else {
                throw new LockedException(
                "Account locked, please try again in " + Duration.between(LocalDateTime.now(), pharmacy.getLockTimer()) + " minutes.");
        ...
        // otherwise return pharmacy optional
